...
RUN apt-get update && apt-get -y upgrade; \
    ...
    tar -xzvf binutils-2.44.tar.gz; \
    tar -xzvf gcc-15.1.0.tar.gz; \
    mkdir build-binutils && cd build-binutils; \
    ../binutils-2.44/configure --target=$TARGET --prefix="$PREFIX" --disable-multilib --disable-nls --disable-werror; \
    make -j$(nproc) && make install; \
    cd $HOME/src && mkdir build-gcc && cd build-gcc; \
    ../gcc-15.1.0/configure --target=$TARGET --prefix="$PREFIX" --disable-multilib --disable-nls --enable-languages=c,c++ --without-headers; \
